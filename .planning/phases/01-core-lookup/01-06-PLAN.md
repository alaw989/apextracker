...
must_haves:
  truths:
    ...
    - "Search history is stored in localStorage after successful search"
  artifacts:
  ...
  key_links:
    - from: "src/App.vue"
      ...
      via: "storeToRefs for searchLoading, data, error"
      pattern: "searchLoading|error"
    - from: "src/App.vue"
      to: "src/stores/search.js"
      via: "useSearchStore().addToHistory() after successful search"
      pattern: "addToHistory"
---

<objective>
Add loading and error states for the search and display flow, plus search history persistence.

Purpose: Provide feedback during API calls and when searches fail. Users need to know when data is loading and receive helpful error messages for invalid searches. Search history is stored for quick re-access (CONTEXT.md requirement). These states complete the user experience for the core lookup feature.

Output: Loading spinner during search, error messages for failed searches, proper state management throughout the flow, and search history persisted to localStorage.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    2. Connect to store states:
       ...
       - From search store: useSearchStore for history (next task)
    ...
    7. Search history integration:
       ...
       - In handleSearch, after successful player fetch:
         * Call searchStore.addToHistory(username, platform)
         * This stores recent searches for quick re-access
       - Only call addToHistory if searchPlayer returns success
    ...
    - Success: player data displays + history stored
  ...
  <verify>
    ...
    6. Verify addToHistory was called: check localStorage for 'apex-search-history'
    7. Test retry: search again after error
    8. Verify all states transition correctly
  </verify>
  <done>
    App shows loading spinner during search, error message for invalid players, player data for successful searches, and stores search history after success
  </done>
</task>
...
- [ ] Search history is stored in localStorage after successful search
- [ ] addToHistory is called in handleSearch after successful fetch
</verification>
...
5. Search history persists across sessions via localStorage
6. User can retry search immediately after error
7. addToHistory is called from handleSearch after successful player fetch
</success_criteria>
