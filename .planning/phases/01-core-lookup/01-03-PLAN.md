...
Output: Working search form with username input, platform icon selector, and search button wired to UI state.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    - Import useSearchStore and useUiStore from Pinia
    - Create local refs: username (''), platform ('origin')
    - Create handleSearch function that:
      - Validates username is not empty
      - Calls searchPlayer from player store (will be implemented in 01-04)
      - For now, just log the search params

    Template:
    - Header with title "CHECK PLAYER RANK AND STATS"
    - SearchInput for username
    - PlatformSelect for platform choice
    - SearchButton to trigger search
  ...
  <verify>
    ...
    4. Verify console logs search parameters
    5. Check responsive layout on mobile viewport
  </verify>
  <done>
    Search form renders with all components, handleSearch function logs parameters, layout matches existing app
  </done>
</task>
