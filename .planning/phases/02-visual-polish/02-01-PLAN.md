...
5. Include legend class-to-gradient fallback mapping for legends without custom art
...
Fallback gradients by legend class (when no custom image exists):
- Assault: linear-gradient(135deg, #ef4444, #b91c1c) — red gradient
- Support: linear-gradient(135deg, #22c55e, #15803d) — green gradient
- Skirmisher: linear-gradient(135deg, #eab308, #a16207) — yellow gradient
- Controller: linear-gradient(135deg, #8b5cf6, #6d28d9) — purple gradient
- Recon: linear-gradient(135deg, #3b82f6, #1d4ed8) — blue gradient
- Unknown class: linear-gradient(135deg, #1e293b, #0f172a) — dark slate default

Legend class mapping (common legends):
- Assault: Bangalore, Catalyst, Revenant, Ash
- Support: Gibraltar, Lifeline, Mirage, Newcastle, Conduit
- Skirmisher: Wraith, Octane, Pathfinder, Horizon, Valkyrie, Fuse
- Controller: Caustic, Wattson, Crypto, Seer, Rampart
- Recon: Bloodhound, Vantage, Crypto
...
- Includes legend class-to-gradient mapping (Assault, Support, Skirmisher, Controller, Recon)
  </verify>
  <done>
Calling getBackgroundForLegend("wraith") returns a valid image path
Calling getBackgroundForLegend("Unknown") returns class-based gradient or default background
  </done>
</task>
...
- .bg-fade-enter-active, .bg-fade-leave-active: transition opacity 0.4s ease-out
...
- Smooth 0.4s crossfade transition between backgrounds
...
2. Crossfade transition is smooth (0.4s duration as specified in CSS)
